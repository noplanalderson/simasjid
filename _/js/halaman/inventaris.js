    ...
    $("#tblBarang").on('click', '.log-barang', function(){
        $('.modal-title').html('Log Kas');

        const hash = $(this).data('id');
        $.ajax({
            url: baseURI + '/log-barang',
            data: {
                    kode_barang: hash, 
            ...
            success: function(data){
                ...
                $.each(data.log, function(index, value){
                    dataTimeline.push('<div class="timeline__item"><div class="timeline__content"><h1 class="mb-2">'+value.aksi+'</h1><p>Tanggal Log: '+value.timestamp+'</p><p>Petugas : '+value.real_name+'</p><p>Nama Barang : '+value.nama_barang+'</p><p>Keterangan : '+value.keterangan+'</p><p>Kuantitas Masuk : '+value.kuantitas_masuk+' '+value.satuan+'</p><p>Kuantitas Keluar : '+value.kuantitas_keluar+' '+value.satuan+'</p></div></div>');
    ...
    $("#tblBarang").on('click', '.delete-btn', function(e){
      e.preventDefault();
